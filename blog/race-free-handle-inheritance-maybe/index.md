...
> *Added on August 20*: Andrew Boyarshin showed me how to save the "steal handles" approach. `NtQueryInformationProcess(ProcessSequenceNumber)` [returns the process sequence number](https://github.com/winsiderss/systeminformer/issues/425#issuecomment-500168121), a monotonic ID that is unique within a boot session, which is enough to avoid the race condition. It's available since [RS4](https://en.wikipedia.org/wiki/Windows_10_version_history#Version_1709_(Fall_Creators_Update)) and, unfortunately, isn't implemented by Wine either, but if you don't care about that, it's a much better option than manipulating process memory!
