#![feature(array_chunks, bigint_helper_methods, portable_simd)]
...
fn sha256_load_six_words(vecs: [u8x32; 4], length_in_bits: u32) -> [u32x4; 16] {
...
fn sha256_adapt_iterated(h: [u32x4; 8]) -> [u32x4; 16] {
    let mut w: [u32x4; 16] = [u32x4::splat(0); 16];
...
#[inline(always)]
fn sha256_core(leading_w: [u32x4; 16]) -> [u32x4; 8] {
    // Initialization
    let mut h0 = u32x4::splat(0x6a09e667);
    let mut h1 = u32x4::splat(0xbb67ae85);
    let mut h2 = u32x4::splat(0x3c6ef372);
    let mut h3 = u32x4::splat(0xa54ff53a);
    let mut h4 = u32x4::splat(0x510e527f);
    let mut h5 = u32x4::splat(0x9b05688c);
    let mut h6 = u32x4::splat(0x1f83d9ab);
    let mut h7 = u32x4::splat(0x5be0cd19);
    ...
    let mut w: [u32x4; 64] = [u32x4::splat(0); 64];
...
fn sha256_store_leading_four_bytes(h: [u32x4; 8]) -> [u32; 4] {
    h[0].to_array()
}

struct ParallelChecksum<'a> {
    queue: ArrayVec<([u8; 21], Box<dyn FnOnce(u32) + 'a>), 4>,
}
...
impl<'a> ParallelChecksum<'a> {
    ...
    fn flush(&mut self) {
        let mut inputs = [u8x32::splat(0); 4];
